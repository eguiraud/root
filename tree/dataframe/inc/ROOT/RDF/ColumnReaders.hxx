...
public:
   ...
   /// Initialize the column reader. Called at the beginning of a processing task.
   virtual void
   Init(unsigned int slot, RDFDetail::RCustomColumnBase *customCol, TTreeReader *r, const std::string &colName) = 0;
...
class R__CLING_PTRCHECK(off) RDefineReader final : public RColumnReaderBase<T> {
   /// The slot this value belongs to. It is set in `SetTmpColumn`.
   ...
   RDFDetail::RCustomColumnBase *fCustomColumn = nullptr;

public:
   void Init(unsigned int slot, RDFDetail::RCustomColumnBase *customColumn, TTreeReader *, const std::string &) final
   {
...
public:
   ...
   RTreeColumnReader() = default;

   void Init(unsigned int, RDFDetail::RCustomColumnBase *, TTreeReader *r, const std::string &colName) final
   {
...
public:
   ...
   RVec<T> &Get(Long64_t) final
   {
      ...
      // TODO Move check to `MakeProxy` once ROOT-10823 is fixed and TTreeReaderArray itself exposes this information
...
template <typename T>
std::unique_ptr<RColumnReaderBase<T>> MakeColumnReader(RDFDetail::RCustomColumnBase *customCol)
{
   using Ret_t = std::unique_ptr<RColumnReaderBase<T>>;

   if (customCol == nullptr)
      return Ret_t(new RTreeColumnReader<T>());

   if (customCol->IsDataSourceColumn())
      return Ret_t(new RDSColumnReader<T>());

   return Ret_t(new RDefineReader<T>());
}

template <typename T>
void SetColumnReader(std::unique_ptr<RColumnReaderBase<T>> &colReader, RDFDetail::RCustomColumnBase *customCol)
{
   auto newColReader = MakeColumnReader<T>(customCol);
                       ...
                       std::index_sequence<S...>, const std::array<bool, sizeof...(S)> &isCustomColumn)
{
   ...
   (void)expander{
      (SetColumnReader(std::get<S>(valueTuple), isCustomColumn[S] ? customColMap.at(colNames[S]).get() : nullptr),
       0)...,
      0};

   // Init is executed as the braced init list is expanded. The final ... expands S.
   (void)expander{
      (std::get<S>(valueTuple)
          ->Init(slot, isCustomColumn[S] ? customCols.GetColumns().at(colNames[S]).get() : nullptr, r, colNames[S]),
       0)...,
      0};
